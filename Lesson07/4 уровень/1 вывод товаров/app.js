...
let prodEl = window.document.querySelector('.products');
let all_buttonEl = window.document.querySelectorAll('button');
all_buttonEl.forEach(function (elem) {
...
function clickHandler(event) {
    ...
    let cat = event.target.getAttribute('data-type');
    showCategory(cat);
}
...
function showCategory(category) {
    let prod = products[category];
    for (let i = 0; i < prod.length; i++) {
        let txt = `<div class="product"><div>${prod[i].name}</div>
                   <img src="${prod[i].imageUrl} "alt="">
                   <div> ${prod[i].price}</div>
                   <a href="https://example.com/producs/${prod[i].id}">Подробнее</a></div>`;
        prodEl.insertAdjacentHTML('afterbegin', txt);
    }
